...
void Rehabilitation_GUI::Update(myo::Hub& hub, MyoController& collector)
{
	while (quit == false) {
		MainMenu(hub, collector);
	}
...
void Rehabilitation_GUI::MainMenu(myo::Hub& hub, MyoController& collector)
{
	...
	//While no buttons have been pressed the following loop runs
	while (continue_func == false)
	{
		...
		SDL_Event Buttonclick;
		SDL_WaitEvent(&Buttonclick);
		//Runs when a button on the keyboard or mouse is pressed
		switch (Buttonclick.type)
		{
		case SDL_MOUSEBUTTONDOWN:
